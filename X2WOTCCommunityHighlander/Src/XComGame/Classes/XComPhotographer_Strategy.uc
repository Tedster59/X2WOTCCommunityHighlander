...
function bool HasPendingHeadshot(const out StateObjectReference UnitRef, optional delegate<OnPhotoRequestFinished> Callback, optional bool bHighPriority=false)
{
	...
	`Log("XComPhotographer_Strategy: HasPendingHeadshot for unit "@UnitRef.ObjectID @"result:" @HasPending,,'TedLog');
...
function AddHeadshotRequest(const out StateObjectReference UnitRef, name LocTag, name CapTag, int ImgWidth, int ImgHeight, delegate<OnPhotoRequestFinished> FinishedDelegate, optional X2SoldierPersonalityTemplate Personality, optional bool bFlushPendingRequests, optional bool bHighPriority)
{
	...
	`Log("XComPhotographer_Strategy: AddHeadshotRequest called for unit" @ Unit.ObjectID @Unit.GetFullName(),,'TedLog');
	...
	{
		`Log("bFlushPendingRequests was true! ",,'TedLog');
	...
	{
		`Log("Headshot not in progress, calling StartHeadshot",,'TedLog');
		StartHeadshot();
	}
	`Log("Existing Headshot already in progress!",,'TedLog');
}
...
private function StartHeadshot()
{
    ...
    {
		`Log("ExcutingReqeust Archetype is loaded",,'TedLog');
    ...
    else
    {
        if( ! ExecutingRequest.bArchetypeIsRequested)
        {
			...
			`Log("ExecutingRequest.bArchetypeIsRequested set to true",,'TedLog');
        }
...
private function StartHeadshotCapture()
{
	...
	`Log("StartHeadshotCapture called",,'TedLog');
...
private function XComUnitPawn CreateUnitPawn(name PawnPlacementActorTag, const out StateObjectReference UnitRef, bool bAsync = false)
{
	...
	if(!UnitStateObject.IsAlien())
	{
		...
		if (UnitStateObject.GetMyTemplate().GetPawnNameFn != none)
		{
			UnitStateObject.kAppearance.nmPawn = UnitStateObject.GetMyTemplate().GetPawnNameFn(EGender(UnitStateObject.kAppearance.iGender));
		}
		else
		{
    ...
    if( bAsync )
    {
	    UnitStateObject.LoadPawnPackagesAsync(self, PlacementActor.Location, PawnRotation, OnPawnLoaded);	
